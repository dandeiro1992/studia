//////////////////////////////////////////////////////////////////////////////////////////
// torus by Majkel (wspolrzedne sferyczne)
//
// sterowanie:
...
double  duzy_promien  = DUZY_PROMIEN;
...
// Prototypy funkcji
void RysujTorus(double R, double r);
void UstawParametryWidoku(int szer, int wys);
void WyswietlObraz(void);
void ObslugaKlawiatury(unsigned char klawisz, int x, int y);

//funkcja rysujaca torusa
void RysujTorus(double R, double r)
{
...
//torus
     glColor3f(1,1,1);
      for(fi = 0; fi <= 2*PI; fi += skok_poz){
            glBegin(GL_QUAD_STRIP);                  
            for(theta = 0; theta <= 2*PI; theta += skok_pion){
                      glVertex3f((R+r*cos(theta))*cos(fi),r*sin(theta),(R+r*cos(theta))*sin(fi));
                      glVertex3f((R+r*cos(theta))*cos(fi+skok_poz),r*sin(theta),(R+r*cos(theta))*sin(fi+skok_poz));
            }
            glEnd();     
     }
}
...
void WyswietlObraz(void)
{
  ...
  RysujTorus(duzy_promien,promien);
